...
public class RandomInstructionRewriter extends InstructionRewriter {
    ...
    private RandomInstructionRewriter(RandomInstructionRewriter source) {
        super(Opcodes.forDexVersion(39));
        opcodeList.addAll(source.opcodeList);
        rebuildOpcodeMap();
    }

    /**
     * Return an isolated worker instance with the same per-run opcode map.
     * InstructionRewriter keeps mutable resolver references, so sharing one
     * instance between DEX workers would corrupt generated reference indexes.
     */
    public RandomInstructionRewriter fork() {
        return new RandomInstructionRewriter(this);
    }

    private void rebuildOpcodeMap() {
        opcodeMap.clear();
