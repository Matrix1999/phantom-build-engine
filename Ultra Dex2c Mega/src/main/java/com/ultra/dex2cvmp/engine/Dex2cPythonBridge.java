 ...
 *
 * Extraction: scripts are unpacked into the active protection job directory and
 * removed with that job. No selected-APK conversion state is retained between
 * runs.
 *
...
public class Dex2cPythonBridge {
    ...
    public DexTranspiler.TranspileResult transpile(
        ...
        return transpile(apkPath, filterText, outputDir, cb, null);
    }

    /**
     * Run DEX2C against the caller's shared extracted-DEX workspace.
     * The legacy APK argument remains available as a fallback for callers that
     * do not yet provide extracted files.
     */
    public DexTranspiler.TranspileResult transpile(
            String apkPath,
            String filterText,
        ...
        try {
            if (cb != null) cb.onProgress("Extracting job-local Ultra Dex2C-VMP scripts…");
            File scriptsDir = extractScriptsForRun(outputDir);

            outputDir.mkdirs();
            resultFile.delete();

            File bridgeScript = new File(scriptsDir, "dex_bridge.py");
            if (!bridgeScript.exists()) {
                result.errors.add("dex_bridge.py missing — reinstall the protector and retry");
                return result;
            }
            ...
            if (filterText != null && !filterText.trim().isEmpty()) {
                // This file belongs to outputDir, which is a unique protection
                // job workspace. It is never placed in the shared app cache.
                filterFile = new File(outputDir, "dex2c_filter.txt");
                if (filterFile.exists() && !filterFile.delete()) {
                    throw new IOException("Unable to replace the job filter file.");
                }
            ...
            List<File> pythonDexFiles = null;
            if (sharedDexFiles != null && !sharedDexFiles.isEmpty()) {
                pythonDexFiles = DexTranspiler.selectDexFilesForFilter(
                        sharedDexFiles, filterText);
                if (pythonDexFiles.isEmpty()) {
                    pythonDexFiles = new ArrayList<>(sharedDexFiles);
                }
                File workspace = pythonDexFiles.get(0).getParentFile();
                if (workspace != null) {
                    cmd.add("--dex-dir");
                    cmd.add(workspace.getAbsolutePath());
                }
                for (File dex : pythonDexFiles) {
                    if (dex != null && dex.isFile()) {
                        cmd.add("--dex");
                        cmd.add(dex.getAbsolutePath());
                    }
                }
                Log.i(TAG, "Using " + pythonDexFiles.size()
                        + " shared DEX file(s) for Python");
            } else {
                // Backwards-compatible path for direct bridge callers.
                cmd.add("--apk");
                cmd.add(apkPath);
            }
    ...
    /**
     * Extract scripts into this protection job only.  The parent outputDir is
     * deleted by ApkProtector in all success, failure, and cancellation paths.
     */
    private File extractScriptsForRun(File outputDir) throws IOException {
        File dest = new File(outputDir, "dex2c_python");
        deleteDir(dest);
        if (!dest.mkdirs()) {
            throw new IOException("Unable to create job-local Python scripts directory.");
        }
        extractScriptsFromBlob(dest);
        Log.i(TAG, "Job-local script extraction complete: " + dest);
        return dest;
    }
    ...
    private static byte[] decryptBlobAsset(Context ctx) throws IOException {
        byte[] raw;
        try (InputStream is = ctx.getAssets().open("python/scripts.blob");
    ...
    /** Extract only dex2c_python/ entries from the assets blob into destBase. */
    private void extractScriptsFromBlob(File destBase) throws IOException {
