...
public class ManifestPatcher {
    ...
    public static byte @NotNull [] parseManifest(byte[] manifestBytes, String proxyAppName) throws IOException {
        return parseManifest(manifestBytes, proxyAppName, false);
    }

    /**
     * Patch the manifest for the Phantom packed APK.
     *
     * Phantom's final ZIP contains compressed native libraries. When the input
     * explicitly requests direct native-library mapping
     * (android:extractNativeLibs="false"), Android rejects that packed APK
     * before launch. Flip only that existing application attribute when the
     * caller is producing the Phantom output; normal DEX2C/VMP output keeps the
     * source manifest unchanged.
     */
    public static byte @NotNull [] parseManifest(byte[] manifestBytes, String proxyAppName,
                                                  boolean forceNativeLibExtraction) throws IOException {
            ...
            if (parser.getName().equals("manifest")) {
                ...
                for (int i = 0; i < size; ++i) {
                    ...
                    if (resId == 0x01010003) {
                        ...
                        off += 8;
                        writeInt(data, off, index);
                    } else if (forceNativeLibExtraction
                            && (resId == 0x010104ea
                            || "extractNativeLibs".equals(parser.getAttributeName(i)))) {
                        // TYPE_INT_BOOLEAN, true. Rewrite in place so the
                        // manifest chunk size and attribute ordering remain
                        // unchanged for the integrity stamp.
                        byte[] data = axml.getData();
                        int attrOff = parser.currentAttributeStart + 20 * i;
                        writeInt(data, attrOff + 8, 0xFFFFFFFF);
                        data[attrOff + 15] = 0x12;
                        writeInt(data, attrOff + 16, 1);
