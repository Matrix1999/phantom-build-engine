...
public class NdkBuilder {
    ...
    private final Object perRunOpcodesLock = new Object();
    private String installedPerRunOpcodesSource;
    private long installedPerRunOpcodesLength = -1L;
    private long installedPerRunOpcodesModified = -1L;
    ...
    public BuildResult compile(File sourceDir, File outputSo, String targetAbi, BuildCallback cb) {
        return compile(sourceDir, outputSo, targetAbi, cb, 0, null);
    }

    /**
     * Compile with an optional per-ABI worker cap and shared global semaphore.
     * The semaphore prevents concurrent ABI builds from exceeding the caller's
     * total native worker budget.
     */
    public BuildResult compile(File sourceDir, File outputSo, String targetAbi,
                               BuildCallback cb, int workerLimit,
                               Semaphore sharedWorkerSlots) {
        ...
        return compileWithClang(resolvedClang, sourceDir, outputSo, targetAbi, cb,
                workerLimit, sharedWorkerSlots);
    }

    /**
     * Publish the randomized VMP opcode header once before parallel ABI builds start.
     *
     * All ABI compilers share {@link #headersDir}. Publishing from each ABI task caused
     * concurrent REPLACE_EXISTING operations against the same DexOpcodes.h on Android.
     * The losing copy could throw even though the other ABI had installed identical
     * bytes. A temporary file plus a synchronized move avoids partial reads and makes
     * an installation failure fatal instead of silently compiling against the static
     * opcode table.
     */
    public void prepareVmpHeaders(File sourceDir, BuildCallback cb) throws IOException {
        ensurePerRunDexOpcodesInstalled(sourceDir, cb, true);
    }

    private void ensurePerRunDexOpcodesInstalled(
            File sourceDir, BuildCallback cb, boolean reportInstalled) throws IOException {
        if (!initialized || headersDir == null) {
            throw new IOException("NdkBuilder is not initialized.");
        }
        if (sourceDir == null) {
            throw new IOException("VMP source directory is missing.");
        }

        File source = new File(sourceDir, "DexOpcodes.h");
        if (!source.isFile() || source.length() <= 0L) {
            throw new IOException("Per-run DexOpcodes.h is missing or empty: "
        ...
        synchronized (perRunOpcodesLock) {
            File destination = new File(headersDir, "DexOpcodes.h");
            boolean alreadyInstalled =
                    sourcePath.equals(installedPerRunOpcodesSource)
                    && sourceLength == installedPerRunOpcodesLength
                    && sourceModified == installedPerRunOpcodesModified
                    && destination.isFile()
            ...
            File temporary = new File(headersDir, "DexOpcodes.h.installing");
            try {
                java.nio.file.Files.copy(
                        source.toPath(),
                        temporary.toPath(),
                        java.nio.file.StandardCopyOption.REPLACE_EXISTING);
                if (!temporary.isFile() || temporary.length() != sourceLength) {
                    throw new IOException("Incomplete per-run DexOpcodes.h copy: expected "
                            + sourceLength + " bytes, got "
                            + (temporary.isFile() ? temporary.length() : 0L));
                }
                ...
                java.nio.file.Files.move(
                        ...
                        destination.toPath(),
                        java.nio.file.StandardCopyOption.REPLACE_EXISTING);
                if (!destination.isFile() || destination.length() != sourceLength) {
                    throw new IOException("Per-run DexOpcodes.h verification failed after install.");
                }

                installedPerRunOpcodesSource = sourcePath;
                installedPerRunOpcodesLength = sourceLength;
                installedPerRunOpcodesModified = sourceModified;
                Log.i(TAG, "Per-run DexOpcodes.h installed once → " + destination);
                if (reportInstalled && cb != null) {
                    cb.onProgress("VMP: installed per-run DexOpcodes.h once before parallel ABI builds");
                }
            } finally {
                if (temporary.exists() && !temporary.delete()) {
                    Log.w(TAG, "Could not remove temporary opcode header: " + temporary);
                }
            }
        }
    }
    ...
    private static String abiToGuardAsset(String abi) {
        switch (abi) {
            case "armeabi-v7a": return "cipher/libcipher_armeabi-v7a.a";
            case "x86_64":      return "cipher/libcipher_x86_64.a";
            case "x86":         return "cipher/libcipher_x86.a";
            default:            return "cipher/libcipher_arm64.a";
        }
    }
     ...
     *
     * Each ABI has its own archive (cipher/libcipher_arm64.a, cipher/libcipher_armeabi-v7a.a,
     * cipher/libcipher_x86_64.a, cipher/libcipher_x86.a) produced by build-libcipher.yml and
     * committed to assets/cipher/. Linking via -Wl,--whole-archive absorbs all guard
    ...
    private File writeGuardJniStub(File dir) {
        ...
        try (java.io.FileWriter fw = new java.io.FileWriter(stub)) {
            fw.write(
                ...
                + "extern \"C\" void fonts_apply_metrics(JNIEnv*);\n"
                + "extern \"C\" int d2c_signer_gate_bind(JNIEnv*);\n"
                + "JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void*) {\n"
                + "    JNIEnv* env = nullptr;\n"
                + "    if (vm->GetEnv(reinterpret_cast<void**>(&env), JNI_VERSION_1_6) != JNI_OK) return -1;\n"
                + "    if (!d2c_signer_gate_bind(env)) return JNI_ERR;\n"
    ...
    private BuildResult compileWithClang(File clangBin, File sourceDir, File outputSo,
        ...
        try {
            ...
            if (files != null) {
                File signerProvider = new File(sourceDir, "d2c_signer_gate.cpp");
                if (!signerProvider.isFile() || signerProvider.length() == 0) {
                    throw new IOException("Native signer provider source is missing: "
                            + signerProvider.getAbsolutePath());
                }
                for (File f : files) {
                    String n = f.getName();
            ...
            // Detect which file owns JNI_OnLoad. A mixed DEX2C + VMP-signer
            // build deliberately contains both generated files; jni_init.cpp
            // always wins and invokes the DEX2C registration helper itself.
            ...
            if (isVmpJniInit) {
                // Avoid two JNI_OnLoad definitions. d2c_registration.cpp remains
                // compiled and is called from GlobalDexConfig's jni_init.cpp.
                generatedFiles.removeIf(f -> "jni_onload.cpp".equals(f.getName()));
                allSrc.removeIf(f -> "jni_onload.cpp".equals(f.getName()));
            } else {
                for (File f : generatedFiles) {
            ...
            // VMP mode: verify that the per-run DexOpcodes.h generated from the
            // RandomInstructionRewriter's opcode map has been installed.
            //
            ...
            if (isVmpJniInit) {
                // ApkProtector prepares this before launching the ABI pool. Keep this
                // synchronized fallback for direct NdkBuilder callers.
                ensurePerRunDexOpcodesInstalled(sourceDir, cb, false);
            }
            ...
            //   cipher/libcipher_arm64.a, cipher/libcipher_armeabi-v7a.a, cipher/libcipher_x86_64.a, cipher/libcipher_x86.a
            // Produced by build-libcipher.yml and committed to assets/cipher/.
            ...
            if (guardA == null) {
                ...
                result.error = "Guard component unavailable: " + assetName + " not found in "
                    ...
                    + "src/main/assets/cipher/, and rebuild.";
                return result;
            }
            ...
            // ── Phase 1: parallel compile → .o  (fresh job, no object reuse) ─
            ...
            if (workerLimit > 0) {
                cores = Math.max(1, Math.min(cores, workerLimit));
            }
            ...
            // Every protection run compiles its own generated source set.  Do not
            // reuse object files or a build-cache manifest from an earlier
            // selection, even if the APK and source names happen to match.
            List<Integer> toCompile = new ArrayList<>(allSrc.size());
            for (int i = 0; i < allSrc.size(); i++) {
                toCompile.add(i);
            }
            ...
            if (cb != null) cb.onProgress(
                ...
                + (sharedWorkerSlots != null ? " (shared ABI worker budget)" : "")
                + " (fresh isolated build; cache disabled)…"
            ...
            for (int idx : toCompile) {
                ...
                futures.add(pool.submit(() -> {
                    ...
                    ObjResult r = runCompileProcessWithSlots(cmd, sharedWorkerSlots);
                    ...
                    if (!r.success && r.log != null &&
                        ...
                        r = runCompileProcessWithSlots(fallbackCmd, sharedWorkerSlots);
                        if (r.success) {
                            if (fcb != null) fcb.onProgress(
            ...
            // Absorb cipher/libcipher_arm64.a (guard) into the target .so.
            ...
            if (linkResult.success && outputSo.exists()) {
                ...
                // Guard is now linked from cipher/libcipher_arm64.a (static archive).
    ...
    /**
     * Run one compiler process while honoring the optional global ABI budget.
     * The permit is held only for the lifetime of clang, not while the result
     * is collected or the next source is queued.
     */
    private ObjResult runCompileProcessWithSlots(List<String> cmd,
                                                  Semaphore sharedWorkerSlots) {
        if (sharedWorkerSlots == null) {
            return runCompileProcess(cmd);
        }

        boolean acquired = false;
        try {
            sharedWorkerSlots.acquire();
            acquired = true;
            return runCompileProcess(cmd);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            ObjResult r = new ObjResult();
            r.error = "Compile interrupted while waiting for a shared worker slot";
            r.log = e.toString();
            return r;
        } finally {
            if (acquired) {
                sharedWorkerSlots.release();
            }
        }
    }
    ...
    private static void patchJniOnload(File f) {
        ...
        try {
            ...
            // Already patched?
            if (src.contains("fonts_register_natives") && src.contains("d2c_signer_gate_bind")) {
