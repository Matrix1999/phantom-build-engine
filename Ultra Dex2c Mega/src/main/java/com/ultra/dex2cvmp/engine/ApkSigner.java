...
public class ApkSigner {
    ...
    public static void sign(Context context, File unsigned, File output) throws Exception {
        ...
        try {
            new com.android.apksig.ApkSigner.Builder(
                ...
                .setOutputApk(output)
                // guard.cpp verifies the raw META-INF certificate entry, so the
                // built-in final signer must retain a V1 certificate block.
    ...
    public static byte[] getDebugP12Bytes(Context context) throws IOException {
        try (InputStream in = context.getAssets().open("keys/debug.p12")) {
