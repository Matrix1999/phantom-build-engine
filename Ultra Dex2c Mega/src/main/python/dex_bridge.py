...
def transpile_dex(dex_path, method_keys, output_dir):
    ...
    if results:
        try:
            _write_d2c_startup_sources(output_dir, results)
        except Exception as registration_error:
            errors.append('registration_error: ' + str(registration_error))
            _debug_log(log_path, 'REGISTRATION_ERROR: ' + traceback.format_exc()[:500])

    _debug_log(log_path, 'Compiled: ' + str(len(results)) + '  Errors: ' + str(len(errors)))
    return {'compiled': results, 'errors': errors, 'error': None}


def _write_d2c_startup_sources(output_dir, compiled):
    """Write the standalone DEX2C JNI bootstrap and RegisterNatives table.

    The Android protection flow imports transpile_dex() instead of executing this
    file's CLI entry point, so this output must be generated here as well as in
    the standalone command-line path.
    """
    if not compiled:
        raise ValueError('cannot generate native registration without compiled methods')

    stale_registration = os.path.join(output_dir, 'd2c_registration.cpp')
    if os.path.exists(stale_registration):
        os.remove(stale_registration)

    jni_onload_path = os.path.join(output_dir, 'jni_onload.cpp')
    with open(jni_onload_path, 'w', encoding='utf-8') as fp:
        fp.write(
            '#include "Dex2C.h"\n'
            ...
            '    return err ? JNI_ERR : JNI_VERSION_1_6;\n'
            '}\n'
        )

    exports = {}
    for _, info in compiled.items():
        class_path = info['cls'][1:-1]
        exports.setdefault(class_path, []).append((
            info['method_name'],
            info['desc'],
            info['jni_name'],
            info['prototype'],
    ...
    lines = ['#include "DynamicRegister.h"\n']
    for class_path in sorted(exports):
        for method in exports[class_path]:
            prototype = method[3].strip()
            if prototype:
                lines.append('extern %s;\n' % prototype)

    lines.append('\nconst char *dynamic_register_compile_methods(JNIEnv *env) {\n')
    lines.append('    if (!env) return "missing_jni_env";\n')
    lines.append('    jclass clazz;\n')
    for index, class_path in enumerate(sorted(exports)):
        methods = exports[class_path]
        lines.append(
            '    clazz = env->FindClass("%s");\n'
            '    if (clazz == nullptr || env->ExceptionCheck()) {\n'
            '        if (env->ExceptionCheck()) env->ExceptionClear();\n'
            '        return "find_class_failed";\n'
            '    }\n'
            % class_path
        )
        entries = ',\n        '.join(
        ...
        lines.append(
            '    if (env->RegisterNatives(clazz, export_method_%d, %d) != JNI_OK || env->ExceptionCheck()) {\n'
            '        if (env->ExceptionCheck()) env->ExceptionClear();\n'
            '        env->DeleteLocalRef(clazz);\n'
            '        return "register_natives_failed";\n'
            '    }\n'
            '    env->DeleteLocalRef(clazz);\n'
            % (index, len(methods))
        )
    ...
    dynamic_register_path = os.path.join(output_dir, 'DynamicRegister.cpp')
    with open(dynamic_register_path, 'w', encoding='utf-8') as fp:
        fp.writelines(lines)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--apk',    required=False, help='Legacy path to input APK or DEX file')
    parser.add_argument('--dex-dir', required=False,
                        help='Shared extracted DEX workspace')
    parser.add_argument('--dex', action='append', default=[],
                        help='Explicit extracted DEX file; may be repeated')
    ...
    # New callers provide the files selected by the Java-side class-index scan.
    # Read those files directly instead of reopening the APK.  The legacy APK
    # path remains for older callers and still performs the cheap raw pre-filter
    # before creating any androguard objects.
    ...
    if args.dex:
        _dex_paths = [p for p in args.dex if os.path.isfile(p)]
        _n_total = len(_dex_paths)
        for _path in _dex_paths:
            _dname = os.path.basename(_path)
            with open(_path, 'rb') as _df:
                _data = _df.read()
            print('Shared DEX: %s (%.0f KB)' % (_dname, len(_data) / 1024))
            sys.stdout.flush()
            _raw_target.append((_dname, _data))
    elif args.dex_dir:
        _dex_names = sorted(
            n for n in os.listdir(args.dex_dir)
            if n.endswith('.dex') and '/' not in n)
        _n_total = len(_dex_names)
        for _dname in _dex_names:
            _path = os.path.join(args.dex_dir, _dname)
            with open(_path, 'rb') as _df:
                _data = _df.read()
            if _has_filter and not any(_cb in _data for _cb in _target_cls_bytes):
                print('Skip %s (%.0f KB) — target classes absent'
                      % (_dname, len(_data) / 1024))
                sys.stdout.flush()
                continue
        ...
        parser.error('one of --dex, --dex-dir, or --apk is required')
    ...
    # ── 5. Write standalone DEX2C jni_onload.cpp ─────────────────────────────
