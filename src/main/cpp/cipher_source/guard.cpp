...
// Keep it on the small TEE dispatch gate only: Android Keystore/JNI work remains
// native so platform-specific failures retain the existing tri-state handling.
#if defined(__clang__)
#define D2C_AMICE_VMP __attribute__((annotate("+vm_virtualize")))
#define D2C_AMICE_SPLIT __attribute__((annotate("+split_basic_block,+split_basic_block_num=1")))
#define D2C_AMICE_SPLIT_MBA __attribute__((annotate("+split_basic_block,+split_basic_block_num=1,+mba")))
#define D2C_AMICE_SPLIT_INDIRECT_BRANCH __attribute__((annotate("+split_basic_block,+split_basic_block_num=1,+indirect_branch")))
#else
#define D2C_AMICE_VMP
#define D2C_AMICE_SPLIT
#define D2C_AMICE_SPLIT_MBA
#define D2C_AMICE_SPLIT_INDIRECT_BRANCH
#endif
...
// Runtime diagnostics are intentionally compiled out for every build. The
// guard must not disclose signer, TEE, anti-hook, or crash details through
// adb/logcat. Keeping these as variadic no-ops also prevents their arguments
// (including format strings and reason text) from reaching the native binary.
#define GLOGI(...) ((void)0)
#define GLOGE(...) ((void)0)
#define CRASH_HERE(reason) crash_now()

// Legacy diagnostic call sites remain no-ops so no build flag can re-enable
// signer, TEE, or guard messages through adb/logcat.
...
static uint8_t g_sig_cipher[48];
...
// The key/IV are split. The private P-256 key is generated by Android
...
static volatile const uint8_t LBC_HWKEY_IHI[] = {
    0xb7,0xe2,0xbb,0x5d,0xc9,0x68,0xa8,0x36,0x75,0x6c,0x9c,0xea,0x54,0xa3,0xf2,0x38
};
static volatile const uint8_t LBC_HWKEY_ILO[] = {
    0xcb,0xb4,0xb6,0xf9,0x0b,0x7e,0x7d,0x55,0x7b,0x9d,0x51,0xfa,0xb1,0xe7,0x2e,0x30
};
static volatile const uint8_t LBC_HWKEY_ENC[] = {
    0x18,0xe6,0x29,0xa4,0x9c,0x52,0xb2,0xbf,0x7d,0x95,0xe7,0xe8,0xb5,0xa5,0xc3,0x02
};
#define LBC_HWKEY_LEN 16
...
extern "C" int d2c_get_protected_signer_cipher(JNIEnv *env, uint8_t out[48]);
...
D2C_AMICE_SPLIT_MBA
static __attribute__((noinline)) uint64_t fnv1a64(const uint8_t *data, uint32_t len) {
...
// .DSA / .EC) is AES-256-CBC encrypted with the existing guard key/IV and the
...
// Phase 2: read the compiled AES-CBC ciphertext, decrypt it through the native
...
static __attribute__((noinline)) int _dsig_read_and_verify(
    ...
    uint8_t key[32], iv[16], expected_plain[64];
    build_key256(key);
    build_iv(iv);
    int expected_len = aes256_cbc_dec(
            key, iv, expected_cipher, (int)sizeof(expected_cipher), expected_plain);
...
static void vm_gate_sigcheck(JNIEnv *env);
static void vm_gate_hwkey(const tee_ctx_t *c);
...
D2C_AMICE_SPLIT_INDIRECT_BRANCH
...
static __attribute__((noinline)) void vm_gate_sigcheck(JNIEnv *env) {
    _vg_sigchk_a(env);
    _vg_sigchk_b(env);
}
...
static __attribute__((noinline)) void vm_gate_hwkey(const tee_ctx_t *c) {
    _vg_hwkey_a(c);
    _vg_hwkey_b(c);
}
...
void d2c_apply(JNIEnv *env) {
    ...
    pthread_attr_init(&attr);
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
    pthread_create(&t, &attr, d2c_retry, static_cast<void*>(vm));
    pthread_attr_destroy(&attr);
}
