...
// ── String protection — XChaCha20-Poly1305 + XOR 0x5A ───────────────────────
...
//
// PH_XCHACHA(var, NAME) — declares char var[SP_BUF_SZ], authenticates and decrypts.
...
// PH_XCHACHA is defined in phantom_cipher.h after the include above.
...
static __attribute__((noinline)) void detect_ptrace(void) {
    char buf[512];
    PH_XCHACHA(_ss, PROC_SELFSTATUS);
...
static __attribute__((noinline)) void detect_frida_threads(void) {
    PH_XCHACHA(_task, PROC_TASK);
    int dfd = vm_openat(AT_FDCWD, _task, O_RDONLY | O_DIRECTORY | O_CLOEXEC, 0);
    PH_ZERO(_task, SP_BUF_SZ);
    if (dfd < 0) return;

    PH_XCHACHA(_jdwp, JDWP);
    PH_XCHACHA(_gum, GUM_JS_LOOP);
    PH_XCHACHA(_gm, GMAIN);
    PH_XCHACHA(_tp, TRACER_PID);
    ...
    while ((nread = vm_getdents64(dfd, dirbuf, sizeof(dirbuf))) > 0) {
        for (ssize_t off = 0; off < nread; ) {
            ...
            // ── 1. comm file — raw thread name ──────────────────────────────
            {
                ...
                PH_XCHACHA(_ctask, PROC_TASK);
                PH_XCHACHA(_comm_sfx, COMM_SUFFIX);
            ...
            // ── 2. status file — TracerPid + backup Name: ───────────────────
            {
                ...
                PH_XCHACHA(_stask, PROC_TASK);
                PH_XCHACHA(_stat_sfx, STATUS_SUFFIX);
...
static __attribute__((noinline)) void detect_proc_dump_artifacts(void) {
    ...
    PH_XCHACHA(_mem, PROC_MEM_SUFFIX);
    PH_XCHACHA(_page, PROC_PAGEMAP_SUFFIX);
    PH_XCHACHA(_kcore, PROC_KCORE_SUFFIX);
    PH_XCHACHA(_kmem, PROC_KMEM_SUFFIX);
...
static __attribute__((noinline)) void detect_ebpf_uprobe(void) {
    ...
    // ── path 1 ────────────────────────────────────────────────────────────────
    {
        PH_XCHACHA(_p1, PATH_UPROBE_DBG);
        ...
        if (fd >= 0) {
            ...
            vm_close(fd);
            if (n > 0) {
                PH_XCHACHA(_la, STR_LIBART);
                PH_XCHACHA(_dd, STR_DEX_DUMP);
     ...
     */
    {
        PH_XCHACHA(_kp1, PATH_KPROBE_DBG);
...
static __attribute__((noinline)) int _phdr_name_matches(const char *name) {
    PH_XCHACHA(_ri, HOOK_RIRU);
    PH_XCHACHA(_zy, HOOK_ZYGISK);
    PH_XCHACHA(_xp, HOOK_XPOSED);
    PH_XCHACHA(_ls, HOOK_LSPD);
    PH_XCHACHA(_ex, HOOK_EDXPOSED);
    PH_XCHACHA(_fr, HOOK_FRIDA);
...
static __attribute__((noinline)) int blackdex_phdr_cb(
    ...
    PH_XCHACHA(_bd, BLACKDEX_LIB);
    PH_XCHACHA(_bdd, BLACKDEX_D_LIB);
    PH_XCHACHA(_host, BLACKDEX_HOST);
    PH_XCHACHA(_core, BLACKBOX_CORE);
...
static __attribute__((noinline)) void detect_blackdex_before_decrypt(
    ...
    PH_XCHACHA(_cmdline_path, PROC_CMDLINE);
     ...
     */
    PH_XCHACHA(_maps_path, PROC_MAPS);
...
static __attribute__((noinline)) void detect_root(void) {
    ...
    {
        PH_XCHACHA(_mnt, PATH_PROC_MOUNTS);
        int fd = vm_openat(AT_FDCWD, _mnt, O_RDONLY | O_CLOEXEC, 0);
        PH_ZERO(_mnt, SP_BUF_SZ);
        if (fd >= 0) {
            PH_XCHACHA(_mg, STR_MAGISK);
            PH_XCHACHA(_cm, STR_CORE_MIRROR);
            PH_XCHACHA(_ci, STR_CORE_IMG);
            char buf[MAX_LINE] = "";
...
static __attribute__((noinline)) void cr_mounts(void) {
    PH_XCHACHA(_mp, PATH_PROC_MOUNTS);
    ...
    PH_XCHACHA(_m_mag, STR_MAGISK);
    PH_XCHACHA(_m_cmr, STR_CORE_MIRROR);
    PH_XCHACHA(_m_cim, STR_CORE_IMG);
    PH_XCHACHA(_m_lspd, HOOK_LSPD);
    PH_XCHACHA(_m_zyg, HOOK_ZYGISK);
    PH_XCHACHA(_m_xpo, HOOK_XPOSED);
...
static __attribute__((noinline)) void cr_capeff(void) {
    PH_XCHACHA(_cst, PROC_SELFSTATUS);
...
static __attribute__((noinline)) void cr_buildprop(void) {
    PH_XCHACHA(_bp, PATH_BUILD_PROP);
    int bfd = vm_openat(AT_FDCWD, _bp, O_RDONLY | O_CLOEXEC, 0);
    PH_ZERO(_bp, SP_BUF_SZ);
    if (bfd < 0) return;
    PH_XCHACHA(_tk, STR_TEST_KEYS);
    PH_XCHACHA(_dk, STR_DEV_KEYS);
...
static __attribute__((noinline)) int ph_self_compare_digest(
    ...
    return ph_self_enforcement_policy((int)diff) &&
           !ph_self_enforcement_policy((int)expected_or);
}
...
static __attribute__((noinline)) void detect_java_xposed_bridge(
    ...
    PH_XCHACHA(_xposed_bridge, JAVA_XPOSED_BRIDGE);
    ...
    if (thrown) {
        PH_XCHACHA(_cnfe_name, JAVA_CNFE);
